...
class GamesFetcherTest {
    ...
    public void getGameContainingKeywordResponseForValidKeyword() throws IOException, InterruptedException {
        ...
        List<Game> result = gamesFetcher.getGameContainingKeyword(keyword);

        assertEquals("testID", result.get(0).getSteamID());
    }
    ...
    public void getGameUsingIDThatDoesNotExist() throws IOException, InterruptedException {
        ...
        when(mockClient.send(any(HttpRequest.class), ArgumentMatchers.<HttpResponse.BodyHandler<String>>any())).thenReturn(mockitoResponse);
